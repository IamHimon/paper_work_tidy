...
class TextCNN_POS(object):
    def __init__(self, vocab_size, embedding_dim, pos_vocab_size, pos_emb_dim, sequence_length, num_classes, filter_sizes,
        ...
        with tf.device('/cpu:0'), tf.name_scope("embedding"):
            ...
            pos = tf.Variable(tf.random_uniform([pos_vocab_size, pos_emb_dim], -1, +1), name='pos')
            ...
            self.pos_emb = tf.nn.embedding_lookup(pos, self.input_pos)

            self.X = tf.concat(2, [self.w_emb, self.pos_emb])
